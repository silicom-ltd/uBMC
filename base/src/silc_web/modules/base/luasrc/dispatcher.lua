...
vendor = util.exec("cat /etc/product/vendor")
...
function authenticator.pam_htmlauth(validator, accs, default)
	local user = luci.http.formvalue("username")
	local pass = luci.http.formvalue("password")

	if user and pass then
		ret, pam_user, user_privil, proto = pam.authenticate(user, pass)
		if ret == "OK" then
			if not util.contains(accs, user) then
				util.append(accs, user)
			end
			if user_privil == "null" then
				local path
				if proto == "radius" then
					path = "/config/radius/static/privilege"
				else
					path = "/config/unix/user/" .. pam_user.."/privilege"
				end
				ret, user_privil = mgmtdclient.query(path)
				if ret ~= "OK" then
					user_privil = "0"
				end
			end
			context.logname = user
			context.user_privil = tonumber(user_privil)
			context.auth_proto = proto
require "nixio"
			nixio.syslog("notice", "Web login as user " .. user)
			return user
		end
	end
...
function dispatch(request)
	...
	if track.sysauth then
		...
		local user, logname, user_privil, auth_proto

		if sdat then
			if not verifytoken or ctx.urltoken.stok == sdat.token then
				user = sdat.user
				logname = sdat.logname
				user_privil = sdat.user_privil
				auth_proto = sdat.auth_proto
			end
		else
...
--[[
				...
				if not user or not util.contains(accs, user) then
					...
					if not sess then
						...
						sauth.write(sid, {
							...
							logname=ctx.logname,
							user_privil=ctx.user_privil,
							auth_proto=ctx.auth_proto,
							ctime=os.time(),
							from=luci.http.getenv("HTTP_X_REAL_IP"),
						})
	...
	local content=io.open("/etc/EULA.txt", "r")
	if content ~= nil then
		local accept = io.open("/config/EULA.accept", "r")
		if accept == nil then
			if c.module ~= "luci.controller.admin.index" then
				local page = build_url("admin/license")
				http.redirect(page)	
				return
			else
				if c.index ~= true then
					if type(c.target) == "function" then
						local page = build_url("admin/license")
						http.redirect(page)	
						return
					else
						if c.target.name ~= "action_license" then
							local page = build_url("admin/license")
							http.redirect(page)	
							return
						end
