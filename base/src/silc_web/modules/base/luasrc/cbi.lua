...
function Table.__init__(self, form, data, ...)
	...
	function datasource.get(self, section, option)
		if tself.data[section] then
			--if not option:find("#") then
				--return tself.data[section][option]
			--end
			local t = tself.data[section]
			for id in option:gmatch("[^#]+") do
				tmp = t[id]
	...
	function datasource.set(self, section, option, value)
		if not tself.data[section] then
			tself.data[section] = {}
		end
		--if not option:find("#") then
			--tself.data[section][option] = value
			--return
		--end
		local t = tself.data[section]
		for id in option:gmatch("[^#]+") do
	...
	function datasource.del(self, section, option)
		--tself.data[section][option] = nil
		return true
	end
...
function DummyValue.cfgvalue(self, section)
	...
	if self.value_map and self.value_map[value] then
...
function DummyValue.mapvalue(self, key, value)
	if not self.value_map then
		self.value_map = {}
	end
...
local dspt = require "luci.dispatcher"

--[[
ListFlag 
]]--
ListFlag = class(ListValue)

function ListFlag.__init__(self, ...)
	ListValue.__init__(self, ...)

	self:value("true", "On")
	self:value("false", "Off")
end


--[[
DummyFlag - a dummyValue component display flag
]]--
DummyFlag = class(DummyValue)

function DummyFlag.__init__(self, ...)
	DummyValue.__init__(self, ...)

	self:mapvalue("true", "<span style='color: green'>Up</span>")
	self:mapvalue("false", "<span style='color: red'>Down</span>")
end

--[[
TfFlag - a flag component value 'true' or 'false'
]]--
TfFlag = class(Flag)

function TfFlag.__init__(self, ...)
	Flag.__init__(self, ...)
...
--[[
DynTable - a dynamic table section
]]--
DynTable = class(Table)

function DynTable.parse(self, readinput)
	self.map.readinput = (readinput ~= false)
	...
	if self.map:submitstate() then
		...
		local cbe_tbl = {}
		local k_tbl = {}
		cbid_tbl = self.map:formvaluetable("cbid."..self.config)
		cbe_tbl = self.map:formvaluetable(FEXIST_PREFIX..self.config)
		for k,v in pairs(cbe_tbl) do
			cbid_tbl[k] = v
		end
		for k,v in pairs(cbid_tbl) do
			-- get the last part seperated by "." as option
			o_pos = k:find("[^.]*$")
			sec = k:sub(1, o_pos-2) 
			opt = k:sub(o_pos, -1)
			if not self.data[sec] then
				self.data[sec] = {}
			end
			--self.data[sec][opt] = v
			if not k_tbl[sec] then
				k_tbl[sec] = ""
			end
		end
		--remove the rec not in submit form
	...
	if self.addremove then
		local crval = CREATE_PREFIX .. self.config
		local rmval = REMOVE_PREFIX .. self.config
		local origin, name
		--remove
		origin, name = next(self.map:formvaluetable(rmval))
		if name then
...
function tbl_deepcopy(t_dest, t_src)
	for key,value in pairs(t_src) do
		if type(value) == "table" then
			t_dest[key] = {}
			tbl_deepcopy(t_dest[key],value)
		else
			t_dest[key]=value
		end
	end
...
PRIVILEGE_MONITOR = dpt.PRIVILEGE_MONITOR
PRIVILEGE_ADMIN = dpt.PRIVILEGE_ADMIN
PRIVILEGE_SUPER = dpt.PRIVILEGE_SUPER
...
MdForm = class(SimpleForm)

function MdForm.__init__(self, config, md_path, acl_level, section_cfg, title, description)
	SimpleForm.__init__(self, config, title, description, nil)
	self.template = "cbi/mdform"
...
function MdForm.section(self, class, ...)
	local obj  = class(self, ...)
	self:append(obj)
	obj.readonly = self.readonly
	return obj
...
function MdForm.set_acl(self)
	self.user = dpt.context.authuser
	self.user_privil = dpt.context.user_privil
	mgmtdclient.set_login(self.user, self.user_privil, dpt.context.auth_proto)
	if self.user_privil >= self.acl_level then
		self.readonly = false
	else
		self.readonly = true
	end
...
function MdForm.get_md_data(self)
	local retcode
	retcode, self.curr_data = mgmtdclient.query_sub(self.md_path)
	if retcode ~= "OK" then
		self.errmessage = translate("Get data error: " .. retcode)
	end
	if type(self.curr_data) ~= "table" then
		self.curr_data = {}
	end

	for i,sec in ipairs(self.section_cfg) do
		if sec.name ~= ROOTNODE and type(self.curr_data[sec.name]) ~= "table" then
			self.curr_data[sec.name] = {}
		end
	end

	if luci.http.formvalue("cbi.commit") then
		tbl_deepcopy(self.orig_data, self.curr_data)
	end
end

function MdForm.cfg_section(self)
	for i, sec in ipairs(self.section_cfg) do
		local s
		if sec.multiple then
			s = self:section(DynTable, self.curr_data[sec.name], translate(sec.tag))
			s.template = "cbi/tblsection"
			s.anonymous = false
		elseif sec.name == ROOTNODE then
			s = self:section(Table, {self.curr_data}, translate(sec.tag))
			s.template = "cbi/nullsection"
		else
			s = self:section(Table, {self.curr_data[sec.name]}, translate(sec.tag))
			s.template = "cbi/nullsection"
		end
		if self.readonly then
			s.addremove = false
		else
			s.addremove = sec.addremove
		end
		s.config = sec.name
		s.map.config = sec.name

		sec.obj = s
	end
end

function MdForm.prepare_all(self)
	self:set_acl()
	self:get_md_data()
	self:cfg_section()
end

function MdForm.get_section_obj(self, sec_name)
	for i, sec in ipairs(self.section_cfg) do
...
function MdForm.handle(self, state, data)
	if state == FORM_VALID then
		if luci.http.formvalue("cbi.commit") then
			if self.pre_commit then
				self:pre_commit()
			end
			for i, sec in ipairs(self.section_cfg) do
				if sec.multiple then
					--remove
					for k,v in pairs(self.orig_data[sec.name]) do
						if sec.addremove and not self.curr_data[sec.name][k] then
							ret = mgmtdclient.delete(self.md_path.."/"..sec.name.."/"..k)
							if ret ~= "OK" then
								self.errmessage = translate(string.format("%s %s delete error: %s!!!!!", sec.name, k, ret))
								return true
							end
						end
					end
					--add
					for k,v in pairs(self.curr_data[sec.name]) do
						if sec.addremove and not self.orig_data[sec.name][k] then
							ret = mgmtdclient.add(self.md_path.."/"..sec.name.."/"..k, self.curr_data[sec.name][k])
							if ret ~= "OK" then
								self.errmessage = translate(string.format("%s %s add error: %s!!!!!", sec.name, k, ret))
								return true
							end
						end
					end
					--modify
					for k,v in pairs(self.curr_data[sec.name]) do
						if self.orig_data[sec.name][k] and type(self.orig_data[sec.name][k]) == "table" then
							local mdf = false
							for k1, v1 in pairs(self.curr_data[sec.name][k]) do
								if type(v1) ~= "table" and self.orig_data[sec.name][k][k1] and self.curr_data[sec.name][k][k1] ~= self.orig_data[sec.name][k][k1] then
									mdf = true
									break
								end
							end
							if mdf then
								ret = mgmtdclient.modify(self.md_path.."/"..sec.name.."/"..k, self.curr_data[sec.name][k])
								if ret ~= "OK" then
									self.errmessage = translate(string.format("%s %s modify error: %s!!!!!", sec.name, k, ret))
									return true
								end
							end
						end
				...
				elseif sec.name == ROOTNODE then
					--static only modify
					local mdf = {}
					for k,v in pairs(self.orig_data) do
						if type(v) ~= "table" and self.curr_data[k] and self.orig_data[k] ~= self.curr_data[k] then
							mdf[k] = self.curr_data[k]
						end
					end
					if next(mdf) then
						ret = mgmtdclient.modify(self.md_path, mdf)
						if ret ~= "OK" then
							self.errmessage = translate(string.format("%s modify error: %s!!!!!", self.md_path, ret))
							return true
						end
					end
				else
					...
					local mdf = {}
					for k,v in pairs(self.orig_data[sec.name]) do
						if type(v) ~= "table" and self.curr_data[sec.name][k] and self.orig_data[sec.name][k] ~= self.curr_data[sec.name][k] then
							mdf[k] = self.curr_data[sec.name][k]
						end
					end
					if next(mdf) then
						ret = mgmtdclient.modify(self.md_path.."/"..sec.name, mdf)
						if ret ~= "OK" then
							self.errmessage = translate(string.format("%s modify error: %s!!!!!", sec.name, ret))
							return true
						end
					end
				end
			end
			if self.post_commit then
				self:post_commit()
			end
			self.message = translate(string.format("Commit successfully!"))
		end
	end
	return true
end
