...
function date(val)
	...
	if #val == 10 then
		...
		mm = val:sub(6,7)
		dd = val:sub(9,10)
		if yy:match("[1-9][0-9][0-9][0-9]") and val:sub(5,5)=="-" and val:sub(8,8)=="-" then
			if (dd:match("0[1-9]") or dd:match("[12][0-9]")) and (mm:match("0[1-9]") or mm:match("1[0-2]")) then
				return true
			elseif (dd == "30" and (mm:match("0[1345789]") or mm:match("1[0-2]"))) then
				return true
			elseif (dd == "31" and (mm:match("0[13578]") or mm:match("1[02]"))) then
				return true
			end
		end
	end
...
function time(val)
	...
	if #val == 8 then
		if val:match("^[0-1][0-9]:[0-5][0-9]:[0-5][0-9]$") or val:match("^2[0-3]:[0-5][0-9]:[0-5][0-9]$") then
			return true
		end
	end
	return false
