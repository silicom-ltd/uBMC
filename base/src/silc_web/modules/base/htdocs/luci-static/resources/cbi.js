...
var cbi_validators = {
	...
	'date': function()
	{
		return (this.match(/^([1-9][0-9]{3})-(((0[13578]|1[02])-(0[1-9]|[12][0-9]|3[01]))|((0[469]|11)-(0[1-9]|[12][0-9]|30))|(02-(0[1-9]|[12][0-9])))$/) != null);
	},

	'time': function()
	{
		return (this.match(/^([0-1][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]$/) != null);
	},
	...
	'usrname': function()
	{
		return (this.match(/^[a-z0-9_-]+$/) != null);
	},
...
function cbi_d_check(deps, node) {
	var reverse;
	var def = false;
		...
		for (var j in deps[i]) {
			if (j == "!reverse") {
				...
				for (i in child) {
					...
					for (j in grandchild) {
						if (grandchild[j].value != undefined) {
							grandchild[j].disabled = !istat;
							if (istat)
								grandchild[j].style.backgroundColor = '';
							else
								grandchild[j].style.backgroundColor = '#bebebe';
						}
					}
				}
				return true;
...
function cbi_d_update() {
	...
	for (var i=0; i<cbi_d.length; i++) {
		...
		if (node && node.parentNode && !cbi_d_check(entry.deps, node)) {
			node.parentNode.removeChild(node);
			state = true;
			if( entry.parent )
				cbi_c[entry.parent]--;
		} else if ((!node || !node.parentNode) && cbi_d_check(entry.deps, node)) {
			if (!next) {
