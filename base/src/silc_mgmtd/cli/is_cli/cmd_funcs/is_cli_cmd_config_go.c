...
int is_cli_cmd_config_go(silc_list* p_token_list)
{
	silc_cli_token* p_token;

	silc_list_for_each_entry(p_token, p_token_list, rl_node)
	{
		if(strcmp(p_token->name, "configure") != 0)
		{
	...
	if(silc_cli_mode_curr_set("configure") != 0)
	{
		silc_cli_err_cmd_set_err_info("change to configure command mode failed");
		return -1;
	}
	return 0;
}
