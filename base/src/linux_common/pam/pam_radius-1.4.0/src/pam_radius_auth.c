...
static int _pam_parse(int argc, CONST char **argv, radius_conf_t *conf)
{
	...
	for (ctrl=0; argc-- > 0; ++argv) {
		...
		if (!strncmp(*argv,"conf=",5)) {
			...
			conf->template_user = (char *) *argv+14;
...
#define PAM_FAIL_CHECK if (retval != PAM_SUCCESS) {goto error; }
	...
	if (response->code == PW_AUTHENTICATION_ACK) {
		retval = PAM_SUCCESS;
		pam_putenv(pamh, "AUTH_PROTOCOL=radius");
		if (config.template_user) {
			char ubuf[128];
			snprintf(ubuf, sizeof(ubuf), "LOGIN_USER=%s", user);
			pam_putenv(pamh, ubuf);
			DPRINT(LOG_DEBUG, "Trying template user: %s", config.template_user);
			if (!pam_modutil_getpwnam(pamh, config.template_user)) {
				DPRINT(LOG_INFO, "Could not find local user for template user: %s, permission denied", config.template_user);
				retval = PAM_PERM_DENIED;
			}
			pam_set_item(pamh, PAM_USER, config.template_user);
			snprintf(ubuf, sizeof(ubuf), "AUTH_USER=%s", config.template_user);
			pam_putenv(pamh, ubuf);
			DPRINT(LOG_DEBUG, "Using template user: %s", config.template_user);
		}
	} else {
